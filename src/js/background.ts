...
function getManifestMapForOrigin(origin: Origin): Map<string, Manifest> {
  ...
  console.log('[*background.ts*] manifestMap: ', manifestMap);
...
function handleMessages(
  ...
  switch (message.type) {
    ...
    case MESSAGE_TYPE.LOAD_COMPANY_MANIFEST: {
      console.log('[*background.ts*] message.rootHash : ', message.rootHash);
      console.log(
        '[*background.ts*] message.otherHashes : ',
        message.otherHashes,
      );
      console.log('[*background.ts*] message.leaves : ', message.leaves);
      console.log('[*background.ts*] message.origin : ', message.origin);
      ...
      validateMetaCompanyManifest(
        ...
        if (validationResult.valid) {
          ...
          console.log(
            '[*background.ts*] case MESSAGE_TYPE.LOAD_COMPANY_MANIFEST | manifest: : ',
            manifest,
          );
    ...
    case MESSAGE_TYPE.RAW_JS: {
      ...
      console.log('[*background.ts*] RAW_JS origin: ', origin);
      ...
      console.log('[*background.ts*] RAW_JS message.rawjs: ', message.rawjs);
      console.log('[*background.ts*] RAW_JS encodedJS: ', encodedJS);
      ...
      crypto.subtle.digest('SHA-256', encodedJS).then(jsHashBuffer => {
        ...
        console.log('[*background.ts*] RAW_JS jsHash: ', jsHash);
