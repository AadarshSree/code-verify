...
export async function validateMetaCompanyManifest(
  ...
  console.log(
    '[*validateMetaCompanyManifest.ts*] rootHash internal: ',
    rootHash,
  );
  ...
  console.log(
    '[*validateMetaCompanyManifest.ts*] megaHash jsonStringify(leaves): ',
    megaHash,
  );
  ...
  console.log('[*validateMetaCompanyManifest.ts*] jsHash: ', jsHash);
  ...
  if (jsHash === otherHashes.main || jsHash === otherHashes.longtail) {
    ...
    combinedHash = combinedHashArray
      ...
      .join('');
    console.log(
      '[*validateMetaCompanyManifest.ts*] combinesHash: ',
      combinedHash,
    );
  } else {
    return {valid: false};
